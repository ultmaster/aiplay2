...
private:
    void drawBoard() {
        ...
        for (int y = 0; y < 4; ++y) {
            printf("[");
            for (int x = 0; x < 4; ++x) {
                fprintf(writer, "%d ", board[x][y].val);
                printf("%d%s", board[x][y].val, x < 3 ? "," : "");
            }
            printf("]%s", y < 3 ? "," : "");
        }
        fprintf(writer, "\n");
    ...
    void moveHori(int x, int y, int d) {
        if (board[x + d][y].val && board[x + d][y].val == board[x][y].val && !board[x][y].blocked &&
            !board[x + d][y].blocked) {
            board[x][y].val = 0;
            board[x + d][y].val *= 2;
        ...
        switch (d) {
            case UP:
                for (int x = 0; x < 4; x++) {
                    int y = 1;
                    while (y < 4) {
                        if (board[x][y].val) moveVert(x, y, -1);
                        y++;
                    }
                }
                break;
            ...
            case LEFT:
                for (int y = 0; y < 4; y++) {
                    int x = 1;
                    while (x < 4) {
                        if (board[x][y].val) moveHori(x, y, -1);
                        x++;
                    }
                }
                break;
...
int main(int argc, char *argv[]) {
    reader = fdopen(atoi(argv[1]), "r");
    writer = fdopen(atoi(argv[2]), "w");
